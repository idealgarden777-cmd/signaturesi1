...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    console.error("[NEYO Voice Token] Missing GEMINI_API_KEY");
  ...
  try {
    ...
    const token = await ai.authTokens.create({
      config: {
        uses: 1, // one token = one new Live connection
        expireTime, // short-lived credential
        liveConnectConstraints: {
          model: MODEL,
          config: {
            sessionResumption: {}, // enables session resumption for longer calls
    ...
    /* -------------------------------------------------
       ...
       ------------------------------------------------- */
    if (!token?.name) {
      console.error("[NEYO Voice Token] Gemini returned no token");
    ...
    /* -------------------------------------------------
       SUCCESS
       ------------------------------------------------- */
    console.log("[NEYO Voice Token] Created", {
      model: MODEL,
    ...
    console.error("[NEYO Voice Token] Failed:", error);
